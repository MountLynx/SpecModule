...
> **范围扩展（2026-08-06）**：本设计按计划落地——`_persist_tick` 存轻量快照
> （剥离 records）+ `fired` 轨迹；`resume()` 改 tick 号回退；`list_checkpoints()`
> 显示 (tick, fired)。曾尝试进一步剥离 `run_state.edges/fire_counts/state`
> （最小快照），但 Graph 自有测试证明快照自包含契约（restore 到新 runner）
> 需要它们，故保留（S3 修正，见冗余清理设计）。详见
> `2026-08-06-redundancy-cleanup-design.md`。
