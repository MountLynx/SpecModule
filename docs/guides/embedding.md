...
- **事件与 records 解耦**（`decouple-embed-events`）——宿主传 `event_bus` 即收 `OutputValidated` / `HarnessFailed` 等事件做反馈，不拖审计与落盘；不传则静默零开销。事件类型清单与订阅模式见 `module_harness/infra/events.py`（类型化 dataclass 事件：harness 6 + script 3 + command 3 + 一致性审核 1）。
